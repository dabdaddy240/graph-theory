...
\begin{proof}
	Let $B \subseteq V(G)$ be a subset of vertices of $G$. Let $G^{B + \alpha}$ be the graph with vertex set $V(G) \cup \{\alpha\}$ for a new vertex $\alpha$ and edge set $E(G) \bigcup_{b \in B} \alpha b$. $B \subseteq V(G)$ is a boundary of $G$ if $G^{B + \alpha}$ is also $K_5$-minor-free. 
	...
	Case 3: $G$ is disconnected. Apply to every component. 
